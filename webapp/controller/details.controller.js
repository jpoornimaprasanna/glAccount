sap.ui.define([
...
], function (Controller, PowerBiControls, JSONModel, MessageToast, Formatter, Spreadsheet, Filter, FilterOperator, BusyDialog) {
	"use strict";
	var topCount;
	return Controller.extend("GlAccountsWiki.GlAccountsWiki.controller.details", {
		...
		tableData: function () {
			...
			this.doAjax(sUrl, "GET", null, function (data) {
					a.close();
					for (var i = 0; i < data.glAccountsDtoList.length; i++) {
						if (data.glAccountsDtoList[i].accountNumber === '0011300320') {
							//data.glAccountsDtoList.splice(i, 1);
							data.glAccountsDtoList[i].accountNumber = "0011300320";
							data.glAccountsDtoList[i].accountDesc = "A/R accred - Noble TH Remediation";
							data.glAccountsDtoList[i].accountDescShort = "A/R accred - Noble";
						}
					}
		...
		onPressExport: function () {
			...
			var oDisplayTableData = this.getView().byId("glTableId").getItems();
			if (oDisplayTableData.length <= 0) {
		...
		downloadToExcel: function () {
			...
			this.getView().setModel(oExportModel, "oExportModel");
			var oTable = this.getView().byId("glTableId");
			var oIndices = oTable.getBinding("items").aIndices;
			var oTableArray = [];
			...
			var oFileName = "GL_Account_Wiki " + "_" + oDate + "T" + oTime;
